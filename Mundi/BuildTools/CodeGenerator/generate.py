...
GENERATED_ABSTRACT_HEADER_TEMPLATE = """// Auto-generated file - DO NOT EDIT!
// Generated by Tools/CodeGenerator/generate.py
...
def generate_header_file(class_info):
    ...
    # Abstract 클래스인 경우 다른 템플릿 사용
    if class_info.is_abstract:
        return GENERATED_ABSTRACT_HEADER_TEMPLATE.format(
...
def generate_implement_class(class_info):
    """IMPLEMENT_CLASS 또는 IMPLEMENT_ABSTRACT_CLASS 매크로 확장 코드 생성"""
    class_name = class_info.name

    # Abstract 클래스인 경우
    if class_info.is_abstract:
        return f"""
// IMPLEMENT_ABSTRACT_CLASS({class_name}) expansion
...
def extract_additional_includes(class_info, all_classes, source_dir=None):
    """UPROPERTY와 UFUNCTION에서 사용되는 타입의 헤더 파일을 자동으로 추출"""
    ...
    for prop in class_info.properties:
        # TArray<Type> 같은 템플릿 타입 처리
        prop_type = prop.type.replace('*', '').replace('const', '').replace('&', '').strip()

        # TArray<> 제거
        import re
        match = re.search(r'TArray\s*<\s*(\w+)', prop_type)
        if match:
